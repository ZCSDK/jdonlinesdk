package com.sobot.online.weight.gif;
...
public class GifView2 extends View implements View.OnTouchListener {
    ...
    private void init(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.SobotGifView2, defStyleAttr, android.R.style.Widget);

        movieMovieResourceId = array.getResourceId(R.styleable.SobotGifView2_sobot_gif, -1);
        array.recycle();
...
//        super.onMeasure(widthMeasureSpec,heightMeasureSpec);
        if (movie != null) {
            movieMeasuredMovieWidth = SobotSizeUtils.dp2px(movie.width());
            movieMeasuredMovieHeight =SobotSizeUtils.dp2px( movie.height());
            ...
            int[] screen = SobotSizeUtils.getScreenWH(getContext());
    ...
    public void displayImage(String url, File saveFile, final GifView2 gifView) {
        ...
        HttpBaseUtils.getInstance().download(url, saveFile, new HttpBaseUtils.FileCallBack() {
            ...
            public void onError(Exception e, String msg, int responseCode) {
                SobotOnlineLogUtils.w("图片下载失败:" + msg,e);
            }
