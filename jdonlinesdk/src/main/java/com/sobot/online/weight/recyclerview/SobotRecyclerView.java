package com.sobot.online.weight.recyclerview;
...
import com.sobot.online.weight.recyclerview.adapter.BH;
import com.sobot.online.weight.recyclerview.refresh.ArrowRefreshHeader;
import com.sobot.online.weight.recyclerview.refresh.IMoreFooter;
import com.sobot.online.weight.recyclerview.refresh.IRefreshHeader;
import com.sobot.online.weight.recyclerview.refresh.LoadingMoreFooter;
import com.sobot.online.weight.recyclerview.refresh.ProgressStyle;
...
public class SobotRecyclerView extends RecyclerView {
    ...
    public SobotRecyclerView(Context context) {
        this(context, null);
    }

    public SobotRecyclerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SobotRecyclerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void refresh() {
        if (pullRefreshEnabled && mLoadingListener != null) {
            mRefreshHeader.setState(ArrowRefreshHeader.STATE_Sobot_app_refreshing);
    ...
    public boolean isRefresh() {
        return isSobot_app_refreshing() || isLoadingMore();
    }
     ...
     * 1.上拉刷新中
     */
    public boolean isSobot_app_refreshing() {
...
//            int head_foot_items_Count = layoutManager.getItemCount();
           ...
           /* if (layoutManager.getChildCount() > 0
                    ...
                    && !isNoMore && mRefreshHeader.getState() < BaseRefreshHeader.STATE_Sobot_app_refreshing) {*/
            if (layoutManager.getChildCount() > 0
                    && lastVisibleItemPosition >= layoutManager.getItemCount() - 1
                    /* && layoutManager.getItemCount() > layoutManager.getChildCount() */ //解决屏幕不满足一屏无法加载更多的问题
                    && !isNoMore && isEnabledScroll && mRefreshHeader.getState() < IRefreshHeader.STATE_Sobot_app_refreshing) {
                isLoadingData = true;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                mLastY = ev.getRawY();
                if (isOnTop() && pullRefreshEnabled && isEnabledScroll) {
                    mRefreshHeader.onMove(deltaY / DRAG_RATE);
                    if (mRefreshHeader.getVisibleHeight() > 0 && mRefreshHeader.getState() < ArrowRefreshHeader.STATE_Sobot_app_refreshing) {
                        return false;
                    }
                }
                break;
            default:
                ...
                if (isTop && pullRefreshEnabled && isEnabledScroll) {
        ...
        public void onChanged() {
            ...
            if (mWrapAdapter != null && mEmptyView != null) {
                ...
                if (mWrapAdapter.getItemCount() == emptyCount) {
                    ...
                    SobotRecyclerView.this.setVisibility(View.GONE);
                } else {
                    mEmptyView.setVisibility(View.GONE);
                    SobotRecyclerView.this.setVisibility(View.VISIBLE);
                }
            }
        }
    ...
    public void setSobot_app_refreshing(boolean Sobot_app_refreshing) {
        if (Sobot_app_refreshing && pullRefreshEnabled && mLoadingListener != null) {
            mRefreshHeader.setState(IRefreshHeader.STATE_Sobot_app_refreshing);
