...
public class KeyboardUtil {
    ...
    public static int getMaxPanelHeight(Context context) {
        ...
        if (MAX_PANEL_HEIGHT == 0) {
            MAX_PANEL_HEIGHT = res.getDimensionPixelSize(R.dimen.sobot_max_panel_height);
        }
    ...
    public static int getMinPanelHeight(Context context) {
        ...
        if (MIN_PANEL_HEIGHT == 0) {
            MIN_PANEL_HEIGHT = res.getDimensionPixelSize(R.dimen.sobot_min_panel_height);
        }
...
//                            "user root view not ready so ignore global layout changed!");
        ...
        private void calculateKeyboardShowing(final int displayHeight) {
            ...
            if (KPSwitchConflictUtil.isHandleByPlaceholder(isFullScreen, isTranslucentStatus,
                ...
                if (maxOverlayLayoutHeight == 0 || SobotWidgetApi.getSwitchMarkStatus(SobotMarkConfig.LANDSCAPE_SCREEN)) {//横屏
                    // non-used.
                    isKeyboardShowing = lastKeyboardShowing;
                } else {
