...
public class PagerGridLayoutManager extends RecyclerView.LayoutManager
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        SobotOnlineLogUtils.i("Item onLayoutChildren");
        SobotOnlineLogUtils.i("Item onLayoutChildren isPreLayout = " + state.isPreLayout());
        SobotOnlineLogUtils.i("Item onLayoutChildren isMeasuring = " + state.isMeasuring());
        SobotOnlineLogUtils.e("Item onLayoutChildren state = " + state);
        ...
        SobotOnlineLogUtils.i("count = " + getItemCount());
    ...
    private void recycleAndFillItems(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        SobotOnlineLogUtils.i("mOffsetX = " + mOffsetX);
        SobotOnlineLogUtils.i("mOffsetY = " + mOffsetY);
        ...
        SobotOnlineLogUtils.e("displayRect = " + displayRect.toString());

        int startPos = 0;                  // 获取第一个条目的Pos
        int pageIndex = getPageIndexByOffset();
        startPos = pageIndex * mOnePageSize;
        SobotOnlineLogUtils.i("startPos = " + startPos);
        ...
        SobotOnlineLogUtils.e("startPos = " + startPos);
        SobotOnlineLogUtils.e("stopPos = " + stopPos);
        ...
        SobotOnlineLogUtils.e("child count = " + getChildCount());
    }
    ...
    public void onScrollStateChanged(int state) {
        SobotOnlineLogUtils.i("onScrollStateChanged = " + state);
        mScrollState = state;
    ...
    private Rect getItemFrameByPosition(int pos) {
        ...
        if (null == rect) {
            ...
            // 状态输出，用于调试
            SobotOnlineLogUtils.i("pagePos = " + pagePos);
            SobotOnlineLogUtils.i("行 = " + row);
            SobotOnlineLogUtils.i("列 = " + col);

            SobotOnlineLogUtils.i("offsetX = " + offsetX);
            SobotOnlineLogUtils.i("offsetY = " + offsetY);
    ...
    private int getPageIndexByOffset() {
        ...
        SobotOnlineLogUtils.i("getPageIndexByOffset pageIndex = " + pageIndex);
        return pageIndex;
    }
    ...
    int findNextPageFirstPos() {
        ...
        SobotOnlineLogUtils.e("computeScrollVectorForPosition next = " + page);
    ...
    int findPrePageFirstPos() {
        ...
        int page = mLastPageIndex;
        page--;
        SobotOnlineLogUtils.e("computeScrollVectorForPosition pre = " + page);
        if (page < 0) {
            page = 0;
        }
        SobotOnlineLogUtils.e("computeScrollVectorForPosition pre = " + page);
    ...
    private void setPageIndex(int pageIndex, boolean isScrolling) {
        SobotOnlineLogUtils.e("setPageIndex = " + pageIndex + ":" + isScrolling);
    ...
    public void smoothScrollToPage(int pageIndex) {
        if (pageIndex < 0 || pageIndex >= mLastPageCount) {
            SobotOnlineLogUtils.e("pageIndex is outOfIndex, must in [0, " + mLastPageCount + ").");
            return;
        }
        if (null == mRecyclerView) {
            SobotOnlineLogUtils.e("RecyclerView Not Found!");
            return;
        }
    ...
    public boolean isFirstPage() {
        SobotOnlineLogUtils.i("getPageIndexByOffset = " + getPageIndexByOffset() + "   mLastPageCount=" + mLastPageCount + ")");
    ...
    public boolean isLastPage() {
        SobotOnlineLogUtils.i("getPageIndexByOffset = " + (getPageIndexByOffset()+1) + "   mLastPageCount=" + mLastPageCount + ")");
    ...
    public void scrollToPage(int pageIndex) {
        if (pageIndex < 0 || pageIndex >= mLastPageCount) {
            SobotOnlineLogUtils.e("pageIndex = " + pageIndex + " is out of bounds, mast in [0, " + mLastPageCount + ")");
            return;
        }
        ...
        SobotOnlineLogUtils.e("mTargetOffsetXBy = " + mTargetOffsetXBy);
        SobotOnlineLogUtils.e("mTargetOffsetYBy = " + mTargetOffsetYBy);
