...
import com.sobot.online.weight.html.SobotCustomTagHandler;
import com.sobot.online.weight.rich.EmailSpan;
import com.sobot.online.weight.rich.MyURLSpan;
import com.sobot.online.weight.rich.PhoneSpan;
...
public class HtmlTools {
    ...
    String imagePositionPath = "/sdcard/Record/";
     ...
     */
    public static Pattern PHONE_NUMBER = Pattern.compile("\\d{3}-\\d{9}|\\d{3}-\\d{8}|\\d{3}-\\d{7}|\\d{4}-\\d{8}|\\d{4}-\\d{7}|1+[34578]+\\d{9}|\\+\\d{2}1+[34578]+\\d{9}|400\\d{7}|400-\\d{3}-\\d{4}|\\d{12}|\\d{11}|\\d{10}|\\d{8}|\\d{7}");
    ...
    private String textImagePath = imagePositionPath;
    ...
    public void loadPic(final TextView textView, String source, final String htmlContent,
        ...
        HttpBaseUtils.getInstance().download(source, file, new HttpBaseUtils.FileCallBack() {
            ...
            public void onError(Exception e, String msg, int responseCode) {
                SobotOnlineLogUtils.i(" 文本图片的下载失败", e);
            }

            @Override
            public void inProgress(int progress) {
                SobotOnlineLogUtils.i(" 文本图片的下载进度" + progress);
            }
        });
    }
    ...
    public void setRichText(TextView widget, String content, int color, boolean showBottomLine) {
        ...
        // 显示表情
        span = SobotInputHelper.displayEmoji(context.getApplicationContext(), span);
        // 显示链接
    ...
    public void setRichText(TextView widget, String content, int color) {
        ...
        if (content.contains("<p>")) {
            content = content.replaceAll("<p>", "").replaceAll("</p>", "<br/>");
        }
            ...
            public Drawable getDrawable(String source) {
                if (!TextUtils.isEmpty(source)) {
                    ...
                    if (new File(fileString).exists()) {
                        SobotOnlineLogUtils.i(" 网络下载 文本中的图片信息  " + fileString + "  eixts");
                        // 获取本地文件返回Drawable
                        drawable = Drawable.createFromPath(fileString);
                        if (drawable != null) {
                            // 设置图片边界
                            SobotOnlineLogUtils.i(" 图文并茂中 图片的 大小 width： "
                        ...
                        SobotOnlineLogUtils.i(fileString + " Do not eixts");
    ...
    public static boolean isHasPatterns(String url) {
        ...
        if (getWebUrl().matcher(url.toString()).matches()) {
            ...
            SobotOnlineLogUtils.i("URL 非法，请输入有效的URL链接:" + url);
            return false;
        }
    }
}